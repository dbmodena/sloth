...
from one_shot_sloth import sloth as one_shot_sloth
from utils import parse_table

dataset = "wikipedia"
candidate_file = "candidates.pkl"
...
def main(mode, r_id=-1, s_id=-1, first_id=-1, num_cand=-1, num_res=1, min_w=0, max_w=math.inf, min_h=0, max_h=math.inf,
    ...
    while iter_id < num_cand:
        ...
        for run_id in range(0, 6):
            ...
            results, metrics = one_shot_sloth(r_tab, s_tab, metrics=metrics, min_a=min_a, verbose=verbose, min_w=min_w,
                                              max_w=max_w, min_h=min_h, max_h=max_h, algo=run_params[run_id]["algo"],
                                              bw=run_params[run_id]["bw"])
