...
def main(mode, r_id=-1, s_id=-1, first_id=-1, num_cand=-1, num_res=1, min_w=0, max_w=math.inf, min_h=0, max_h=math.inf,
    ...
    if mode == "s":
        ...
        # num_tabs = len([tab for tab in os.listdir(tab_dir_path)])
        num_tabs = len(file_names[dataset])
        candidates = [(i, j) for i in range(0, num_tabs) for j in range(i + 1, num_tabs)]
        # with bz2.BZ2File(cand_path, "rb") as input_file:
    ...
    while iter_id < num_cand:
        ...
        if cand[0] in tab_cache.keys():
            ...
            with bz2.BZ2File(tab_dir_path + str(file_names[dataset][int(cand[0])]) + ".pkl", "rb") as in_file:
        ...
        if cand[1] in tab_cache.keys():
            ...
            with bz2.BZ2File(tab_dir_path + str(file_names[dataset][int(cand[1])]) + ".pkl", "rb") as in_file:
                tab_tuples = pkl.load(in_file)
                in_file.close()
            tab_content = [list(tup) for tup in tab_tuples]
        ...
        r_tab = [col for col in r_tab if len(set(col)) > 1]
        s_tab = parse_table(s_obj["content"], s_obj["num_columns"], s_obj["num_header_rows"])
        s_tab = [col for col in s_tab if len(set(col)) > 1]
        ...
        keys = [cand_id, (file_names["uni_dwh"][cand[0]], file_names["uni_dwh"][cand[1]]),
                [[(r_obj["content"][0][cols[0]], s_obj["content"][0][cols[1]]) for cols in res[0]] for res in results],
        ...
        # Save the detected composite keys in the dedicated file
        if len(keys) > 0:
            res_key_path = res_key_path_base.split("_")
            res_key_path = res_key_path[0] + "_" + str(first_id) + "_" + res_key_path[1]
            res_key_path = res_dir_path + res_key_path
            if mode == "m":
                with bz2.BZ2File(res_key_path, "ab") as output_file:
                    pkl.dump(keys, output_file)
                    output_file.close()
            else:
                if verbose:
                    print(keys)
